...
class FilletIcon
{
	...
	public function create()
	{
		...
		else if($this->text)
		{
			...
			$borderColor = imagecolorallocate($resource,255,0,0);
			
			$textSize = $this->textSize;//字体大小
			
			$box = imagettfbbox($textSize,0,$this->font,$this->text);
			$box_width = max(abs($box[2] - $box[0]),abs($box[4] - $box[6]));  
			$box_height = max(abs($box[7] - $box[1]),abs($box[5] - $box[3]));  
			
			//echo $box_width  .':'.  $box_height;exit;
			
			//$font_x = $this->iconWidth/2 - $box_width/2;
			//$font_y = $this->iconHeight/2 + $box_height/2;
			$font_x = $this->iconWidth/2 - $box_width/2;
			$font_y = $this->iconHeight/2 - $box_height/2 + ($textSize + 35);
			
			
			
			
			imagettftext($resource, $textSize, 0, $font_x, $font_y, $textColor, $this->font,$this->text);
			...
			imageline($resource,$left_bottom_x,$left_bottom_y,$right_bottom_x,$right_bottom_y,$borderColor);  
			imageline($resource,$right_bottom_x,$right_bottom_y,$right_top_x,$right_top_y,$borderColor);
			imageline($resource,$right_top_x,$right_top_y,$left_top_x,$left_top_y,$borderColor);
			imageline($resource,$left_top_x,$left_top_y,$left_bottom_x,$left_bottom_y,$borderColor);  
